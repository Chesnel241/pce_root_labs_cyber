...
/** @typedef {{ id:string, email:string, username:string, xp:number, createdAt:string }} PublicUser */
 ...
 * @param {{ id:string, email:string, username:string, xp:number|string, created_at:Date|string }} row
 * @returns {PublicUser}
 */
...
export async function login({ email, password }) {
  const normalizedEmail = email.trim().toLowerCase();
  const result = await query(
    `SELECT id, email, username, password_hash, xp, created_at
